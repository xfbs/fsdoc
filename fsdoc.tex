...
Just like Linux, macOS also supports file attributes, called \emph{flags}. These are as documented in Table \ref{tbl:macosflags}. The attributes \emph{arch}, \emph{sappnd} and \emph{schg} can only be set and unset by the superuser. As long as \gls{sip} is active, the \emph{restricted} flag cannot be set or unset by anyone.
